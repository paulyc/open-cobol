...
\item \textbf{Solaris support}: fixed errors when compiling in Solaris 10.
\end{itemize}
...
\item \textbf{New compiler configuration options}: display-special-fig-consts, free-redefines-position (\fr{211}), line-col-zero-default
\end{itemize}

\paragraph{Environment division}
\begin{itemize}
\item \textbf{ASSIGN clause}: re-enable use of linkage section or BASED items (\bug{421}).
\item \textbf{CURRENCY phrase}: emit error when CURRENCY SIGN other than ``\$'' is entered.
\end{itemize}
...
\begin{itemize}
\item \textbf{WHEN-COMPILED function}: fixed incorrect timezone missing sign and containing nonsense when negative (\bug{436}).
\end{itemize}
