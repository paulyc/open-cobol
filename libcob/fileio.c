 ...
 * See isam/bdb.c and isam/lmdb.c respectively. 
 */
#undef WITH_DB
#undef WITH_LMDB
...
cob_settings	*cobsetptr = NULL;

unsigned int	eop_status = 0;
unsigned int	check_eop_status = 0;
int		cob_vsq_len = 0;

struct file_list	*file_cache = NULL;

char		*file_open_env = NULL;
char		*file_open_name = NULL;
char		*file_open_buff = NULL;

char		*runtime_buffer = NULL;
...
#ifdef WITH_ANY_ISAM
int indexed_open		(cob_file *, char *, const int, const int);
int indexed_close	(cob_file *, const int);
int indexed_start	(cob_file *, const int, cob_field *);
int indexed_read		(cob_file *, cob_field *, const int);
int indexed_read_next	(cob_file *, const int);
int indexed_write	(cob_file *, const int);
int indexed_delete	(cob_file *);
int indexed_rewrite	(cob_file *, const int);
#endif
...
cob_delete_file (cob_file *f, cob_field *fnstatus)
{
	...
	if (f->organization != COB_ORG_INDEXED) {
		...
		unlink (file_open_name);
	        ...
	        cob_field_to_string (f->assign, file_open_name, (size_t)COB_FILE_MAX);
	        int status = fileio_funcs[(int)f->organization]->unlink?
			fileio_funcs[(int)f->organization]->unlink (f, file_open_name)
			:
...
cob_init_fileio (cob_global *lptr, cob_settings *sptr)
{
	...
	//// instead: cob_init_fileio_epilogue();
